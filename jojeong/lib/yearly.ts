...
const YEAR_TEXT: Record<GodGroup, string> = {
  ...
  식상: "그대의 말과 재주가 밖으로 드러나는 해이옵니다. 새 일을 벌이기 좋으나, 말 한마디가 화근이 되기도 하옵니다.",
...
export function yearPreview(p: Pillars) {
  ...
  const lead =
    verdict === "대길"
      ? `병오년의 불(火) 기운이 곧 그대의 용신이니, 몇 해에 한 번 오는 큰 해이옵니다.`
      : verdict === "조심"
        ? `병오년의 불(火) 기운이 그대의 용신인 ${chart ? ELEMENT_KO[chart.yong] : ""} 기운을 누르니, 한 해 내내 발밑을 살피셔야 하옵니다.`
        : verdict === "길"
          ? "병오년의 기운이 그대의 사주를 알맞게 받쳐 주니, 순풍에 돛을 단 해이옵니다."
          : "병오년의 기운이 그대와 크게 부딪치지도 크게 돕지도 않으니, 하기 나름인 해이옵니다.";
