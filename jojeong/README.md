...
- `lib/sillok.ts` + `lib/episodes.ts`: 가상 실록 생성(같은 사주는 항상 같은 실록). 요약 대신 **장면형 에피소드 6장**: 세자 시절 일화 → 첫 번째 사건 → 위기와 해결 → 별명이 생긴 사건 → 야사 → 마지막 날. 일화·첫 사건·해결 방식·별명 사건·유언은 일간(왕 유형)별로 달라 왕의 성격이 행동으로 드러남. 조정의 영의정·간신·유배·일반 신하 이름이 등장인물로 들어감. 향년은 24~88세(중앙 56세)로 뽑고 조선 27왕 실제 향년과 비교해 수명 순위를 매김. 평균 수명 46.1세·영조 82세는 황상익 교수 연구 인용(문화일보). 에피소드 문구 수정은 `lib/episodes.ts`에서
...
가상 실록(6장 이야기·존호·재위·향년·수명 순위·사관의 평)은 전부 무료. 유료 상품은 "조선의 전하"를 **지금 현실**로 이어 준다.
